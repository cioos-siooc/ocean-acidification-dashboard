    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                SET status='pending_process', checksum=%s, attempts=attempts+1, last_attempt=NOW()
                WHERE id=%s
                """,
                (checksum, run_id),
            )
        ...
        try:
            ...
            with conn.cursor() as cur:
                for rec in outputs:
                    ...
                    cur.execute(
                        """
                        INSERT INTO nc_jobs (dataset_id, variable_id, start_time, end_time, status, nc_path, checksum, attempts, last_attempt)
                        VALUES (%s, %s, %s, %s, 'pending_image', %s, %s, 0, NOW())
                        ON CONFLICT (dataset_id, variable_id, start_time, end_time)
                        DO UPDATE SET status = EXCLUDED.status, nc_path = EXCLUDED.nc_path, checksum = EXCLUDED.checksum, last_attempt = NOW()
                        """,
                        (ds_id, var_id, start_time, end_time, path, checksum),
                    )
