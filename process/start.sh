...
docker compose --env-file .env.prod -f docker-compose.prod.backend.yml run --rm process uv run MAIN.py compute --workers 2 && sleep 5
...
docker compose --env-file .env.prod -f docker-compose.prod.backend.yml run --rm process uv run MAIN.py image --workers 2
