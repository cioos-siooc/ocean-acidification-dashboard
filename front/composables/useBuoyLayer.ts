...
async function loadBuoyImage(map: any): Promise<void> {
    ...
    return new Promise<void>((resolve) => {
        ...
        img.onload = () => {
            ...
            map.addImage(IMAGE_ID, ctx.getImageData(0, 0, 30, 30), { sdf: true });
            resolve();
        };
...
export async function addBuoyLayer(
    ...
    map.addLayer({
        ...
        paint: {
            'icon-color': ['case', ['==', ['get', 'active'], true], '#FFD700', '#888888'],
            'icon-opacity': 0.95,
        },
    });
