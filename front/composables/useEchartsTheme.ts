...
/**
 * Register the ECharts dark theme directly, bypassing the UMD wrapper in dark.js
 * which fails in Vite/ESM context (root.echarts is undefined when this===undefined).
 */
export function registerEchartsDarkTheme() {
  if (registered) return

  const contrastColor = '#B9B8CE'
  const backgroundColor = '#222' // '#100C2A'
  const axisCommon = () => ({
