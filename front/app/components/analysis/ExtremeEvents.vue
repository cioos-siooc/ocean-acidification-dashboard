...
const events = computed(() => detectExtremeEvents(seasonalSeries.value, climatology.value, direction.value, minDurationDays.value, maxGapDays.value))
...
function render() {
  ...
  const baselinePoints = maskedSeries
    .map(d => {
      ...
      const doy = (month === 2 && day === 29) ? CUM_DAYS_NONLEAP[1] + 28 : CUM_DAYS_NONLEAP[month - 1] + day
      const clim = climByDoy.get(doy)
      return [d.time, clim ? (direction.value === 'above' ? clim.p90 : clim.p10) : null] as [string, number | null]
    })
...
watch([events, direction], () => nextTick(render))
