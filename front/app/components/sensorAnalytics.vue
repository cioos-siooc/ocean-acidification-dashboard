...
function renderOverlayChart(series: { year: number; data: SeriesPoint[] }[]) {
  ...
  chartInstance.setOption({
    ...
    yAxis: {
      type: 'value',
      name: `${varName.value} (${primaryStat.value})`,
      nameLocation: 'middle',
      nameGap: 50,
      axisLabel: { fontSize: 10, color: '#ccc', formatter: (value: number) => Number(value.toFixed(3)).toString() },
      min: 'dataMin',
      max: 'dataMax',
    },
...
// Auto-fetch on sensor, variable, depth, or statistic change — but only while this tab is visible.
watch([selectedSensor, variable, depth, primaryStat], scheduleAutoRun)
...
function currentSignature(): string {
  return JSON.stringify({ id: selectedSensor.value?.id, variable: variable.value, depth: depth.value, stat: primaryStat.value })
}
