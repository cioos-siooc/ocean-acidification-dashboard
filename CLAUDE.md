...
- **API** (`api/modules/posthog_helpers.py`): `capture_event(http_request, event, properties)`, configured via `POSTHOG_API_KEY`/`POSTHOG_HOST`. Called from `sensorTimeseries`, `depthProfile`, `extractTimeseries`, `extract_climateTimeseries`, `getMinMax`, `getProfile`, `getEval`, and `analysis/timeseries` — tile-serving and the `/admin/sync*` routes are deliberately excluded (too high-volume / not user behavior).
