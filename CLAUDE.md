...
**Process CLI** (dev: the `process` container or uv; prod: `docker compose -f docker-compose.prod.process.yml --env-file .env.process.remote exec scheduler uv run python -m SSC.cli …` — see Process Pipeline below):
```bash
...
**Prefect (monitoring + schedule).** `SSC/flows.py` — the only module that imports Prefect, so the CLI works with no server — wraps `pipeline_steps()` as the `oceaneco-ssc-pipeline` flow (deployment `OceanECO-SSC`, tag `oceaneco` — prefixed because the Prefect server is shared across apps), one task run per step, served by the `scheduler` service (`python -m SSC.flows`) — dev to the local `prefect` service, prod to the shared server — on cron `RUN_CRON` (default `0 */3 * * *`) with `RUN_LIMIT`/`RUN_WORKERS` as the flow's `limit`/`workers` (prod defaults 10/30, matching the old host cron's `SSC.cli run --workers 30`; dev 10/4), one run at a time — a run that comes due while one is still going is cancelled (`CANCEL_NEW`), not queued. The `SalishSeaCast.*`/`nc2tile` loggers reach each task's log tab via `PREFECT_LOGGING_EXTRA_LOGGERS`. Sweep stages mark rows `failed_*` without raising, so each run ends with a `oceaneco-ssc-status` markdown artifact (7-day per-status counts + rows that failed during the run) and is marked Failed if any row did. Ad-hoc runs (a `date`, `force`) are triggered from the UI's "Run → custom". `RUN_SCHEDULE_PAUSED` is re-applied on every scheduler start (defaults: **paused in dev**, since `.env.dev` points at the production ClickHouse; unpaused in prod); stop `scheduler` for a pause that holds. `prefect==` in `process/pyproject.toml` must match dev's server image tag, and be ≤ the shared server's version. `docker-compose.prod.process.yml` has **no `process` service** — `scheduler` (`tools` profile) is the only pipeline container, running each scheduled run in-process, and doubles as the CLI (`exec scheduler uv run python -m SSC.cli ...`, or `run --rm scheduler …` while it's stopped); there is no local server: `scheduler` reports to the shared, always-on one at `PREFECT_API_URL` (`https://prefect.cioospacific.ca/api`, basic auth via `PREFECT_AUTH_STRING`; must run Prefect ≥ the pin). `PREFECT_API_URL` is required (`:?`), since the old localhost fallback turned a forgotten `--env-file` into an opaque connection-error restart loop — so every prod compose command needs `--env-file`. `scheduler` replaces the host cron that used to run `SSC.cli run`, and a code update means rebuilding/recreating `scheduler`. Don't start a manual `SSC.cli run` while a scheduled one is in progress — both would claim the same pending rows. After a rename of the flow/deployment, the old one lingers on the server (serve never deletes) — delete the old flow in the UI.
